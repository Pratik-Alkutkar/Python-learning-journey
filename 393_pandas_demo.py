...
def dataframe_demo(): 
    ...
    print(frame1)
    frame2 = frame.reindex(columns=['MH', 'MP', 'TN', 'GA'])
    print("frame2:\n", frame2)

    print("original frame:\n", frame)
    print("Dropping entries from axes")
    ...
    print("frame:\n", frame)
    frame = frame.drop(index=['GA'])
    print("frame:\n", frame)
    frame = frame.drop(columns=['C2'])
    print("frame:\n", frame)

    sr = pd.Series([1, 2, 3], index=[2, 0, 1])
    print("sr:\n", sr)

    # 2 -> 1, 0 -> 2, 1 -> 3
    # subscript works as per explicit indexing 
    # not sequential container indexing 
    print("sr[0]:", sr[0])
    print("sr[1]:", sr[1])
    print("sr[2]:", sr[2])

    # 2 -> 1, 0 -> 2, 1 -> 3
    # loc[i] works as per explicit indexing 
    # not sequential container indexing 
    print("sr.loc[0]:", sr.loc[0])
    print("sr.loc[1]:", sr.loc[1])
    print("sr.loc[2]:", sr.loc[2])

    # 2 -> 1, 0 -> 2, 1 -> 3
    # iloc[i] works as per sequential indexing 
    # irrespective of the nature of explicit indexing
    # explicit index may contain integer objects or 
    # objects of other types, iloc will always work 
    # because it concerns itself with sequential indexing 
    print("sr.iloc[0]:", sr.iloc[0])
    print("sr.iloc[1]:", sr.iloc[1])
    print("sr.iloc[2]:", sr.iloc[2])

    # You can experiment loc and iloc with frames as well !

    print("Arithmetic & Data alignment")
    
    sr1 = pd.Series([1.1, 2.2, 3.3, 4.4], index=['a', 'b', 'd', 'e']) 
    sr2 = pd.Series([5.5, 6.6, 7.7, 8.8, 9.9], index=['a', 'c', 'e', 'f', 'g'])
    print("sr1:\n", sr1)
    print("sr2:\n", sr2)
    sr3 = sr1 + sr2 
    print("sr3:\n", sr3)
    ...
    frame4 = frame1.add(frame2, fill_value=0.0)
    print("frame4:\n", frame4)
    frame5 = frame4.reindex(index=sorted(list(frame4.index)))
    print("frame5:\n", frame5)
    #frame1 - frame2 
    #frame * frame2 
    #frame1 / frame2 
    #frame1 // frame2 
    #frame1 ** frame2 
    # rs_frame = frame1.sub(frame2, fill_value=fv)
    # rs_frame = frame1.mul(frame2, fill_value=fv)
    # rs_frame = frame1.div(frame2, fill_value=fv)
    # rs_frame = frame1.floordiv(frame2, fill_value=fv)
    # rs_frame = frame1.pow(frame2, fill_value=fv)

    print("Application of ufuncs on frame:") 
    frame = pd.DataFrame(np.arange(12).reshape((4, 3)), 
                         columns = ['C1', 'C2', 'C3'], 
                         index = ['MH', 'MP', 'TN', 'GA'])
    print("frame:\n", frame)
    rs_frame = np.sqrt(frame)
    print("rs_frame:\n", rs_frame)

    # Refer to all ufuncs in numpy
    # they call can be applied as follows 
    # rs_frame = np.ufunc(frame)
    # original frame object does not undergo any change 
